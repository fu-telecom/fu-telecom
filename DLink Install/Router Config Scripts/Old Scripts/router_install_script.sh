#Fuck You Telecom Install Script - Flipside 2016
#passwd n23n23129

if [ $# -lt 4 ] ; then
        echo "Command requires arguments. <server_number> <skip_pkgs (1/0)> <chan 2.4ghz> <chan 5ghz>"
        exit 127
fi
...
echo "        option ports '0t 1 2 3 4'" >> /etc/config/network
echo "" >> /etc/config/network
echo "config switch_vlan" >> /etc/config/network
echo "        option device 'switch0'" >> /etc/config/network
