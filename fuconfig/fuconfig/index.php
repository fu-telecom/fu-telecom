...
<div class="container-fluid">
	<div class="row">
		<div class="col"><h1 class="display-4">Master List</h1></div>
	...
	<div class="row mx-2">
		<div class="col">
			...
			<div class="row group-header">
				<div class="col-sm-3"><h4><?= $currentOrg->org_name ?></h4></div>
				...
				<div class="col"><?=$editOrgButton->GetAnchorButtonHTML()?>
					...
					<?php } ?>
					<a class="btn btn-success float-right m-1" href="phone-inventory-assignment.php?org_id=<?= $currentOrg->org_id ?>" role="button">Add Inventory Phone</a>
					<a class="btn btn-success float-right m-1" href="router-inventory-assignment.php?org_id=<?= $currentOrg->org_id ?>" role="button">Add Router</a>
				</div>
			</div>

			<div class="row">
				<div class="col-auto"><h3>Phone List:</h3></div>
			</div>
		...
		?>
			...
			<div class="row bg-secondary rounded text-light ml-4 mt-1">
				...
				<?php if ($phone->phone_is_deployed == 0) { ?>
					<button id="btnDeployed<?=$phone->phone_id?>" onClick="updateDeployed(<?=$phone->phone_id?>)" class="btn btn-warning p-4 m-1 float-right">Not Deployed</button>
				<?php } else if ($phone->phone_is_deployed == 1) {  ?>
					<button id="btnDeployed<?=$phone->phone_id?>" onClick="updateDeployed(<?=$phone->phone_id?>)" class="btn btn-danger p-4 m-1 float-right">Deployed</button>
				<?php } else { ?>
					<button id="btnDeployed<?=$phone->phone_id?>" onClick="updateDeployed(<?=$phone->phone_id?>)" class="btn btn-success p-4 m-1 float-right">Returned</button>
				<?php } ?>
				</div>
				<div class="col">
					<div class="row align-items-center ">
						...
						<div class="col">
							<button id="btnAddExistingNumber" onClick="addExistingNumber(<?=$phone->phone_id ?>)" class="btn btn-primary p-1 m-1 float-right">Add Existing Number</button>
							<button id="btnAddNewNumber" onClick="addNewNumber(<?=$phone->phone_id ?>)" class="btn btn-primary p-1 m-1 float-right">Add New Number</button>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col">
							...
							<button id="btnReloadPhone" onClick="reloadSCCPPhone(<?=$phone->phone_id?>)" class="btn btn-info">Reload SCCP Phone</button>
							<button id="btnReloadPhone" onClick="restartSCCPPhone(<?=$phone->phone_id?>)" class="btn btn-info">Restart SCCP Phone</button>
							<button id="btnRedoPhone" onClick="redoSCCPPhone(<?=$phone->phone_id?>)" class="btn btn-info">Redo In Asterisk</button>
							<?php } ?>
							<button id="btnDeletePhone<?=$phone->phone_id?>"
								onClick="removePhone(<?=$phone->phone_id?>)"
			...
			<div class="row">
				<div class="col-auto"><h3>Router List:</h3></div>
			</div>
...
<div class="modal fade" id="addNumberModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">

</div>

<div class="modal fade" id="editRouterModal" tabindex="-1" role="dialog" aria-labelledby="routerEditTitle" aria-hidden="true">

</div>

<div class="modal fade" id="routerUpdateModal" tabindex="-1" role="dialog" aria-labelledby="routerUpdateTitle" aria-hidden="true">
	<?php include('router-reload-modal.php'); ?>
</div>

<div class="modal fade" id="processPhonesModal" tabindex="-1" role="dialog" aria-labelledby="processingModalTitle" aria-hidden="true">
