...
?>
	...
	if ($routerList->GetCount() > 0) {  ?>
		...
		foreach($routerList->GetList() as $router) {
			...
			if ($router->channel_24 == $router->channel_24_current and
			...
			<div class="row bg-secondary rounded text-light ml-4 mt-1">
				<div class="col-auto">
					<?php if ($router->router_is_deployed == 0) { ?>
						<button id="btnRouterDeployed<?=$router->router_id?>" onClick="updateDeployedRouter(<?=$router->router_id?>)" class="btn btn-warning p-2 m-1 float-right">Not Deployed</button>
					<?php } else if ($router->router_is_deployed == 1) {  ?>
						<button id="btnRouterDeployed<?=$router->router_id?>" onClick="updateDeployedRouter(<?=$router->router_id?>)" class="btn btn-danger p-2 m-1 float-right">Deployed</button>
					<?php } else { ?>
						<button id="btnRouterDeployed<?=$router->router_id?>" onClick="updateDeployedRouter(<?=$router->router_id?>)" class="btn btn-success p-2 m-1 float-right">Returned</button>
					<?php } ?>
				</div>
				...
				<div class="col">
					<div class="row justify-content-end px-4 pt-1">
						<a id="btnChangeChannels" class="btn btn-success p-1 m-1" onClick="editRouter(<?=$router->router_id?>)" href="#">Set Channels</a>
						<a id="btnUpdateRouter" class="btn btn-info p-1 m-1" onClick="processRouterUpdate(<?=$router->router_id?>)" href="#">Update Router</a>
						<a id="btnRemoveRouter" class="btn btn-warning p-1 m-1" onClick="removeRouter(<?=$router->router_id?>)" href="#">Remove Router</a>
					</div>
				</div>
